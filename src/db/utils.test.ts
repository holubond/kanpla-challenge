...
test('childrenOf', async () => {
    ...
    const allNodes = Object.entries(data.groups)
        .concat(Object.entries(data.locations))
        .map(([id, node]) => ({ ...node, id }))
    ...
    const expectedNodes = allNodes.filter((node) =>
        expectedIds.includes(node.id)
    )
...
test('includeBranch', async () => {
    ...
    const expectedGroups = {
        denmark: data.groups['denmark'],
        aarhus: data.groups['aarhus'],
    }
    const expectedLocations = {
        'aarhus-technologies': data.locations['aarhus-technologies'],
        'aarhus-office-hub': data.locations['aarhus-office-hub'],
    }

    // act
    let targetTree: DbTree = {
        groups: {},
        locations: {},
