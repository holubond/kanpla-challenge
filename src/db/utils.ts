...
export function parseTree(unsafeTree: DbTree): DbTree {
    const safeTree: DbTree = {
        groups: {},
        locations: {},
        partnerId: unsafeTree.partnerId,
    }

    forLocations: for (const [locationId, location] of Object.entries(
        unsafeTree.locations
    )) {
        const locationParents = Object.keys(location.parents)

        if (locationParents.length > 1) {
            console.warn(
                `Location ${locationId} is not a valid node - too many parents`
            )
            continue
        }
        ...
        while (ancestorId !== undefined) {
            ...
            if (ancestor === undefined) {
                console.warn(
                    `Location ${locationId} has an ancestor ${ancestorId} that was not found`
                )
            ...
            if (ancestorParents.length > 1) {
                console.warn(
                    `Group ${ancestorId} is not a valid node - too many parents`
                )
                continue forLocations
            }
 ...
 */
export function includeBranch(
    subtreeRoot: DbNodeWithId,
    targetTree: DbTree,
    allNodes: DbNodeWithId[]
) {
    // Include subtree
    ...
    while (ancestorId !== undefined) {
        const ancestor = allNodes.find(
            (node) => node.id === ancestorId
        ) as DbNodeWithId // Expects a valid `allNodes` tree
        targetTree.groups[ancestorId] = {
            name: ancestor.name,
