...
export class InMemoryLocationRepo implements LocationRepo {
    ...
    async getLocationsInGroupsByName(name: string): Promise<DbTree> {
        ...
        const allNodes = Object.entries(fullTree.groups)
            .concat(Object.entries(fullTree.locations))
            .map(([id, node]) => ({ id, ...node }))
        const roots = allNodes.filter(
            (node) => Object.entries(node.parents).length === 0
        )

        // Use lowercase unhyphenised form of search
        name = searchForm(name)

        let filteredTree: DbTree = {
            groups: {},
            locations: {},
            partnerId: fullTree.partnerId,
        }
